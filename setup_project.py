#!/usr/bin/env python3
"""
Template setup script for initializing Python projects from GitHub template.

This script replaces template variables with actual values in-place and sets up the project.
"""
...
def rename_paths(base_path: Path, replacements: dict) -> None:
...
def setup_project() -> None:
    ...
    # Work in the current directory (template is already created by GitHub)
    ...
    for root, _dirs, files in os.walk(target_dir):
        for file in files:
            ...
            # Skip the setup script itself and hidden files
            if (
                file != "setup_project.py"
                and not file.startswith(".")
                and file_path.suffix in [".py", ".md", ".txt", ".toml", ".yml", ".yaml"]
            ):
                replace_in_file(file_path, replacements)

    print(f"\n🎉 Project {project_name} setup successfully!")
